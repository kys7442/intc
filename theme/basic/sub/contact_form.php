	...
	<div class="form_box">
		...
		<div class="input_col_box dfbox">
			<div class="input_col dfbox">
				...
				<select name="wr_1" id="i_item5" class="tbody" onchange="updateSubSelect()">
					<option value="" disabled selected>구분선택</option>
					<option value="종합건설"<?php echo ($depth1 == 1) ? " selected" : "";?>>종합건설</option>
					<option value="창호"<?php echo ($depth1 == 2) ? " selected" : "";?>>창호</option>
					<option value="인테리어"<?php echo ($depth1 == 3) ? " selected" : "";?>>인테리어</option>
					<option value="부분공사"<?php echo ($depth1 == 5) ? " selected" : "";?>>부분공사</option>
				</select>
			</div>
			<div class="input_col dfbox">
...
// 240411 disable 속성추가
const selectBox = document.getElementById('i_item5');

document.addEventListener('DOMContentLoaded', function() {
  const currentPage = window.location.href;

  if (currentPage.includes("construction_inquiry")) {
    selectBox.value = "종합건설";
    disableSelect("종합건설");
  } else if (currentPage.includes("windows_inquiry")) {
    selectBox.value = "창호";
    disableSelect("창호");
  } else if (currentPage.includes("interior_inquiry")) {
    selectBox.value = "인테리어";
    disableSelect("인테리어");
  } else if (currentPage.includes("inquiry_etc")) {
    selectBox.value = "부분공사";
    disableSelect("부분공사");
  }
	
});

/* [OSJ : 2024-04-26] 구분값 전달 오류로 스크립트 변경 */
function disableSelect(name) {
//   selectBox.disabled = true;
	$(selectBox).find("option").attr("disabled", true);
	$(selectBox).find("option[value='"+name+"']").attr("disabled", false);
}
